# Generated by Django 3.2.20 on 2024-06-13 17:44

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion
import django.utils.timezone


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('seqr', '0052_rnaseqspliceoutlier_rank'),
    ]

    operations = [
        migrations.CreateModel(
            name='PubEvidenceNote',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('guid', models.CharField(db_index=True, max_length=30, unique=True)),
                ('created_date', models.DateTimeField(db_index=True, default=django.utils.timezone.now)),
                ('last_modified_date', models.DateTimeField(blank=True, db_index=True, null=True)),
                ('note_type', models.CharField(choices=[('N', 'note'), ('F', 'feedback')], max_length=1)),
                ('note', models.TextField(blank=True, default='')),
                ('gene_id', models.CharField(max_length=20)),
                ('evidence_id', models.CharField(max_length=100)),
                ('note_status', models.CharField(choices=[('N', 'none'), ('V', 'verified'), ('S', 'submitted')], default='N', max_length=1)),
                ('created_by', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='+', to=settings.AUTH_USER_MODEL)),
            ],
        ),
    ]
